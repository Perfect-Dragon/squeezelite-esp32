...
void displayer_local_title(const char *title) {
    ...
    char text[HEADER_SIZE + 1];

    strncpy(text, title, HEADER_SIZE);
    text[HEADER_SIZE] = '\0';

    xSemaphoreTake(displayer.mutex, portMAX_DELAY);

    const int title_height = GDS_GetHeight(display) / 3;

    // Clear only the upper metadata section
    GDS_ClearWindow(
        display,
        0,
        0,
        GDS_GetWidth(display) - 1,
        title_height - 1,
        GDS_COLOR_BLACK
    );

    // Put the first text line approximately in the middle of the 80 px area
    GDS_TextSetFontAuto(
        display,
        1,
        GDS_FONT_LARGE,
    ...
    // Shorten long titles until they fit
    while (strlen(text) > 3 &&
           GDS_GetTextWidth(display, 1, 0, text) >
               GDS_GetWidth(display) - 12) {
